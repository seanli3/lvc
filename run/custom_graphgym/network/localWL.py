...
class LocalWLGNN(torch.nn.Module):
    def __init__(self, dim_in, dim_out):
        ...
        self.beta1 = nn.ParameterList([nn.Parameter(torch.ones(1)*0.) for _ in range(max_path_length)])
        self.beta2 = nn.ParameterList([nn.Parameter(torch.ones(1)*0.) for _ in range(max_path_length)])
        self.beta3 = nn.ParameterList([nn.Parameter(torch.ones(1)*0.) for _ in range(max_path_length)])
    ...
    def forward(self, batch):
        ...
        for hop in range(1, self.max_path_length+1):
            h_v = (1+self.beta1[hop-1])*h[batch['agg_scatter_index_'+str(hop-1)]] + x[batch['agg_node_index_'+str(hop-1)]]
            h = x.scatter_reduce(
