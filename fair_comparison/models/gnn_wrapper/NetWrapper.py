...
class NetWrapper:
    ...
    def _train(self, train_loader, optimizer, clipping=None):
        ...
        if self.classification:
            return acc_all / sum(map(lambda d:len(d), train_loader)), loss_all / sum(map(lambda d:len(d), train_loader))
        else:
    ...
    def classify_graphs(self, loader):
        ...
        if self.classification:
            return acc_all / sum(map(lambda d:len(d), loader)), loss_all / sum(map(lambda d:len(d), loader))
        else:
