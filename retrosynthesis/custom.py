...
class SynthonCompletion(tasks.Task, core.Configurable):
    ...
    def _cat(self, graphs):
        ...
        for k in keys:
            # Added code
            if 'node reference' in meta_dict[k]:
                data_dict[k] = torch.cat(
                    [graphs[i].data_dict[k] + i * graphs[i - 1].num_nodes.sum() for i in range(len(graphs))])
                tmp = []
                for i in range(len(graphs)):
                    if i > 1:
                        tmp.append(graphs[i].data_dict[k] + i * graphs[i - 1].num_nodes.sum())
                    else:
                        tmp.append(graphs[i].data_dict[k])
            else:
    ...
    def _apply_action(self, graph, action, logp):
        ...
        is_reaction_center = \
        functional._extend(graph.is_reaction_center, graph.num_nodes, is_reaction_center, has_new_node)[0]
    ...
    def predict_reactant(self, batch, num_beam=10, max_prediction=20, max_step=20):
        ...
        for i in range(2):
            try:
                delattr(result, 'agg_scatter_index_' + str(i))
            except:
                pass
            try:
                delattr(result, 'agg_node_index_' + str(i))
            except:
                pass
        result = result[order]
